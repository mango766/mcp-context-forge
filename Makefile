...
.PHONY: install-db
install-db: venv
	@$(UV_BIN) pip install --python "$(VENV_DIR)/bin/python" '.[redis,postgres]'

.PHONY: install-dev
install-dev: venv
	@$(UV_BIN) pip install --python "$(VENV_DIR)/bin/python" --group dev .
...
update:
	...
	@$(UV_BIN) pip install --python "$(VENV_DIR)/bin/python" -U --group dev .
